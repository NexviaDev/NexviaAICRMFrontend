...
export default function GoogleChat() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchMessages(selectedSpaceId, true), 5000);
  ...
  const selectedDisplayName = selectedSpaceId
    ? (spaceDisplayNames[selectedSpaceId] ?? selectedSpace?.displayName ?? selectedSpaceId)
    : '';
      ...
      <div className="google-chat-body">
        <aside className="google-chat-spaces">
          <div className="google-chat-spaces-header">
            ...
            <button type="button" className="google-chat-new-chat-btn" aria-label="새 채팅">
          ...
          <div className="google-chat-space-list-wrap">
            {loading ? (
              ...
              <p className="google-chat-empty">참여 중인 스페이스가 없습니다.</p>
            ) : (
              <ul className="google-chat-space-list">
                {spaces.map((space) => {
                  const id = spaceIdFromName(space.name);
                  const displayName = spaceDisplayNames[id] ?? space.displayName ?? space.name ?? id;
            ...
            <>
              <header className="google-chat-main-header">
                ...
                <div className="google-chat-main-header-actions">
                  <button type="button" className="google-chat-header-action-btn" aria-label="검색">
                    <span className="material-symbols-outlined">search</span>
                  </button>
                  <button type="button" className="google-chat-header-action-btn" aria-label="통화">
                    <span className="material-symbols-outlined">call</span>
                  </button>
                  <button type="button" className="google-chat-header-action-btn" aria-label="더보기">
