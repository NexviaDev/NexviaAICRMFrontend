/**
 * PWA 단일 Service Worker (파일명·경로: /firebase-messaging-sw.js)
 * - Workbox: 오프라인 캐시
 * - Firebase: onBackgroundMessage → self.registration.showNotification (앱 종료 시 OS 알림)
 */
...
/** vite closeBundle 시 치환 — VITE_API_URL, VITE_FIREBASE_WEB_* (미치환 시 SW 로드 실패) */
const NEXVIA_PUSH_API_BASE = '__NEXVIA_PUSH_API_BASE__';
const BAKED_FIREBASE_CONFIG = __FIREBASE_CONFIG_JSON__;
...
function initFirebaseWithConfig(cfg) {
  ...
  messaging.onBackgroundMessage((payload) => showCrmNotificationIfAllowed(payload));
  firebaseBackgroundHandlerReady = true;
  return true;
}

async function loadConfigAndInitFirebase() {
  if (initFirebaseWithConfig(BAKED_FIREBASE_CONFIG)) return true;
  try {
...
self.addEventListener('notificationclick', (event) => {
  ...
  if (!targetUrl && data.eventId) {
    targetUrl = `/calendar?modal=event&eventId=${encodeURIComponent(data.eventId)}`;
  }
  if (!targetUrl && (data.type === 'announcement' || data.notificationId)) {
    targetUrl = '/notification';
  }
  if (!targetUrl) targetUrl = '/notification';
